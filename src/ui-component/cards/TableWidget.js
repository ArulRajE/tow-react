...
export default function EnhancedTable({ settings }) {
    ...
    React.useEffect(() => {
        ...
        const getData = () => {
            ...
            analysisService
                .getAll(startTime, format(rangeEnd, endTime), page, rowsPerPage)
                .then(async (results) => {
    ...
    }, [rangeStart, rangeEnd, refreshInterval, deactivateRefresh, page, rowsPerPage, historicalData, historicalStart, historicalEnd]);
        ...
        <Grid item xs={12} sm={6} lg={12} spacing={gridSpacing}>
            <Box sx={{ backgroundColor: '#FFFFFF', paddingTop: '10px' }}>
                                ...
                                <IconButton color="primary" style={{ paddingTop: '4px', paddingBottom: '0px' }}>
                                    ...
                                    {allRecords && (
                                        <ExcelFile
                                            ...
                                            filename={`Analysis Table ${format(rangeStart, 'Y-MM-d HH:mm')} - ${format(
                                                rangeEnd,
                                                'Y-MM-d HH:mm'
                                            )}`}
                                        >
                ...
                <TableContainer>
                    <AverageBlock
                        LocalstartTime={
                            historicalData === 'yes' ? format(historicalStart, 'Y-MM-d HH:mm') : format(rangeStart, 'Y-MM-d HH:mm')
                        }
                        LocalendTime={historicalData === 'yes' ? format(historicalEnd, 'Y-MM-d HH:mm') : format(rangeEnd, 'Y-MM-d HH:mm')}
                        settings={settings}
                    />
                </TableContainer>
