...
import { tagCompletedServices } from '_services/tagCompleted.services';
...
export default function EnhancedTable({ settings, tag }) {
    ...
    React.useEffect(() => {
        ...
        analysisService
            .getAll(tag.LocalstartTime, tag.LocalendTime, page, rowsPerPage)
            .then(async (results) => {
                ...
                const { data: all } = await analysisService.getAll(tag.LocalstartTime, tag.LocalendTime, 0, totalRecords);
                setAllRecords([...all]);
            })
            .catch((err) => {
                    ...
                    <Grid container justifyContent="space-between" alignItems="center" spacing={gridSpacing}>
                        <Grid item xs={12} sm={8}>
                            <Typography variant="subtitle1" component="p">
                                Tag <Chip label={tag.tagName} /> Start Time : <Chip label={tag.LocalstartTime} /> - End Time :{' '}
                                <Chip label={tag.LocalendTime} />
                            </Typography>
                        </Grid>
                        <Grid item xs={12} sm={4} sx={{ textAlign: 'right' }}>
                            <Tooltip title="Export to Excel">
                                <IconButton color="primary">
                                    {/* <ExplicitOutlined /> */}
                                    {allRecords && (
                                        <ExcelFile element={<ExplicitOutlined />} filename={tag.tagName}>
                                            <ExcelSheet data={allRecords.reverse()} name={tag.tagName}>
