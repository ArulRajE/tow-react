...
export default function EnhancedTable({ settings }) {
    ...
    const getFilteredValue2 = (ele, value) => {
        if (ele.columnName === 'LocalendTime' || ele.columnName === 'LocalstartTime') {
            return format(new Date(value), 'Y-MM-d HH:mm');
        }
        if (value === null) {
            return '0.00';
    ...
    React.useEffect(() => {
        ...
        const getData = () =>
            monitorServices
                ...
                .catch((err) => {
                    toast.error('Error in loading tag summary page ');
                });
                        ...
                        <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                            ...
                            <Tooltip title="Export to Excel">
                                <IconButton color="primary" style={{ paddingTop: '4px', paddingBottom: '0px' }}>
                                    {/* <ExplicitOutlined /> */}
                                    {rows && (
                                        <ExcelFile element={<ExplicitOutlined />} filename="Tag Summary">
                                            <ExcelSheet data={rows} name="Tag Summary">
                        ...
                        <TableBody>
                            {stableSort(rows, getComparator(order, orderBy))
                                ...
                                .map((row, index) => {
                                    ...
                                    if (row.tagName === 'Standard Deviation') {
                                        return (
                                            <StyledTableRow
                                                hover
