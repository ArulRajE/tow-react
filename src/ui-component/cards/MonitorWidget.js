...
export default function EnhancedTable({ settings }) {
    ...
    const { rangeStart, rangeEnd, refreshInterval, deactivateRefresh } = useSelector((state) => {
    ...
    const getSettings = async (
        refresh = false,
        intervalStartTime1 = intervalStartTime,
        intervalEndTime1 = intervalEndTime,
        cumulativeIntervalMinutes1 = cumulativeIntervalMinutes
    ) => {
        ...
        monitorServices.getColumns().then(async (response) => {
            ...
            data.forEach(async (item) => {
                if (item.type === 'interval') {
                    ...
                    const { data } = await analysisService.average(intervalStartTime1, intervalEndTime1);
    ...
    const dropDownChange = async (value, col) => {
        if (col.type === 'interval') {
            ...
            const { data } = await analysisService.averageByInterval(diff);
            const h = monitorData;
            h[99] = data;
            setMonitorData({ ...monitorData });
