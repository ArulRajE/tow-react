...
export default function EnhancedTable({ settings }) {
    ...
    React.useEffect(() => {
        ...
        analysisService
            .hourlyAverage(startTime, endTime, intervalHour)
            .then((results) => {
                console.log(results);
    ...
    }, [rangeStart, rangeEnd, refreshInterval, deactivateRefresh, historicalData, historicalStart, historicalEnd]);

    const onSubmitInterval = async (value, intervalHour) => {
        // console.log(value);
        analysisService
            .hourlyAverage(value.start, value.end, intervalHour)
            .then((results) => {
                // console.log(results);
                // alert(JSON.stringify(results));
                const { data } = results;
                    ...
                    <Grid container justifyContent="space-between" alignItems="center" spacing={1}>
                        ...
                        <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }} justify="space-around" spacing={4}>
                            <Box sx={{ m: 0.5 }}>
                                                                ...
                                                                <Grid item>
                                                                    ...
                                                                    <Button
                                                                        onClick={(event) => {
                                                                            ...
                                                                            // dropDownChange(`${startDate} ${startTime}:00`, headCell);
                                                                            onSubmitInterval(
                                                                                {
                                ...
                                {/* <Tooltip title="Copy" color="primary">
                                <IconButton>
                                    <FileCopyIcon />
                                </IconButton>
                            </Tooltip>
                            <Tooltip title="Print" color="primary">
                                <IconButton>
                                    <PrintIcon />
                                </IconButton>
                            </Tooltip> */}
                                <Tooltip title="Export to Excel">
                                    <IconButton color="primary" style={{ paddingTop: '4px', paddingBottom: '0px' }}>
                                        ...
                                        {rows && (
                                            <ExcelFile
                                                element={<ExplicitOutlined />}
                                                filename={`Hourly Analysis Table ${format(rangeStart, 'Y-MM-d HH:mm')} - ${format(
                                                    rangeEnd,
                                                    'Y-MM-d HH:mm'
                                                )}`}
                                            >
                                                <ExcelSheet data={rows.reverse()} name="Hourly Analysis Table">
